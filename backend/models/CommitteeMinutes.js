/**
 * CommitteeMinutes Model - Data access layer for committee meeting minutes operations.
 */
...
exports.findAll = async (status, pageNum, limitNum, safeSort, safeOrder) => {
  ...
  const countRes = await pool.query(
    `SELECT COUNT(*) FROM committee_minutes m ${whereClause}`,
    params
  );
...
exports.deleteById = async (id) => {
  return pool.query('DELETE FROM committee_minutes WHERE id = $1', [id]);
};

exports.findByCommitteeId = async (committeeId) => {
  return pool.query(
    `SELECT m.*, u.name AS created_by_name
     FROM committee_minutes m
     LEFT JOIN users u ON u.id = m.created_by
