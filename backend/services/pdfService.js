...
function writeFooter(doc) {
  ...
  for (let i = pages.start; i < pages.start + pages.count; i++) {
    ...
    // Anchor footer close to the physical page bottom (not the content bottom margin).
    // Keep a small inset so PDFKit never overflows and creates an extra page.
    const footerBottomInset = 18;
    const footerY = doc.page.height - footerBottomInset;
