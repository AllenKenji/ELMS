...
exports.generatePdf = async (req, res) => {
  try {
    const ordinance = await ordinanceService.getOrdinanceById(req.params.id);
    const committeeReport = await ordinanceService.getCommitteeReport(req.params.id).catch(() => null);
    let settings = null;
    try {
    ...
    const proposerSignature = ordinance?.proposer_id
      ? await userService.findSignatureById(ordinance.proposer_id)
      : await userService.findSignatureByName(ordinance?.proposer_name);
...
exports.assignCommittee = async (req, res) => {
  try {
    const { committee_id, meeting_date, meeting_time, secretary_user_id, secretary_name, secretary_email } = req.body;
    const result = await ordinanceService.assignCommittee(
      req.params.id,
      committee_id,
      req.user.id,
      { meeting_date, meeting_time, secretary_user_id, secretary_name, secretary_email }
    );
    res.json(result);
  } catch (err) {
...
exports.secondReading = async (req, res) => {
  try {
    ...
    const result = await ordinanceService.conductSecondReading(
      req.params.id, session_id, discussion_notes, presiding_officer, req.user.id, selected_recording_url
    );
    res.json(result);
  } catch (err) {
