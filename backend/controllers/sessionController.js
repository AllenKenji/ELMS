/**
 * Get all committee reports for a session.
 * GET /sessions/:id/committee-reports
 */
exports.getCommitteeReports = async (req, res) => {
  try {
    const reports = await require('../services/sessionService').getSessionCommitteeReports(req.params.id);
    res.json(reports);
  } catch (err) {
    console.error('Get session committee reports error:', err);
    res.status(500).json({ error: 'Error fetching committee reports for session' });
  }
};
/**
