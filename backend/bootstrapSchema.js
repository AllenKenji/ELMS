...
async function ensureCommitteeMeetingsBaseSchema() {
  await pool.query(`
...
async function ensureCommitteeMinutesBaseSchema() {
  await pool.query(`
  ...
  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_committee_minutes_committee_id
...
async function ensureVotingSchema() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS voting_sessions (
      ...
      voting_type VARCHAR(50) DEFAULT 'yes_no',
...
async function ensureTemplatePreferencesSchema() {
  await pool.query(`
  ...
  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_template_pref_user_type
      ON measure_template_preferences(user_id, measure_type);
    CREATE INDEX IF NOT EXISTS idx_template_pref_favorite
      ON measure_template_preferences(is_favorite);
    CREATE INDEX IF NOT EXISTS idx_template_pref_last_used
      ON measure_template_preferences(last_used_at);
  `);
}
...
async function bootstrapSchema() {
  ...
  await ensureCommitteeMeetingsBaseSchema();
  await ensureCommitteeMinutesBaseSchema();
  ...
  await ensureTemplatePreferencesSchema();
}
