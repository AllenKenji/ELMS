...
import '../../styles/ResolutionWorkflow.css';

function getLatestEndedMeeting(meetings, resolutionId) {
  return meetings.find((meeting) => meeting.resolution_id === resolutionId && meeting.ended);
}
...
const STAGES = [
  ...
  { key: 'EFFECTIVE', label: '13. In Effect', icon: '⚖️', desc: 'Resolution is now in full effect' },
];
...
export default function ResolutionWorkflow({ resolutionId, resolution, committeeMeetings = [], onStatusUpdate, onMeetingCreated }) {
  const { user } = useAuth();
  ...
  const fetchVotingStatus = useCallback(async () => {
    try {
      ...
      const res = await api.get(`/resolutions/${resolutionId}/voting-status`);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [wsRes, sessRes, commRes] = await Promise.all([
        api.get(`/resolutions/${resolutionId}/workflow-status`),
        api.get('/sessions'),
        api.get('/committees'),
      ]);
  ...
  useEffect(() => {
    const stage = workflowStatus?.resolution?.reading_stage;
    if (stage === 'THIRD_READING_VOTING' || stage === 'THIRD_READING_VOTED') {
      fetchVotingStatus();
    }
  }, [workflowStatus?.resolution?.reading_stage, fetchVotingStatus]);
  ...
  const handleActionClick = (action) => {
    ...
    if (action === 'committee-report') {
      const endedMeeting = getLatestEndedMeeting(committeeMeetings, resolutionId);
      setForm({
        recommendation: '',
        report_content: '',
  ...
  const handleSubmitAction = async () => {
    ...
    try {
      ...
      if (activeAction === 'submit-to-vice-mayor') {
        ...
        const stageNow = String(workflowStatus?.resolution?.reading_stage || resl?.reading_stage || '').trim().toUpperCase();
        const readingPhase = stageNow === 'ASSIGN_SECOND_SESSION' || stageNow === 'RECORD_SECOND_SESSION' || stageNow === 'COMMITTEE_REPORT_SUBMITTED'
          ? 'second'
          : 'first';

        await api.post(`/resolutions/${resolutionId}/admin/override-session`, {
        ...
        await api.post(`/resolutions/${resolutionId}/committee-report`, body);
        selfHandled = true;
      ...
      if (!selfHandled) {
        await api.post(`/resolutions/${resolutionId}/${activeAction}`, body);
      }
  ...
  if (loading) return <div className="resolution-workflow"><div className="loading">Loading legislative workflow...</div></div>;
  ...
  const resl = workflowStatus?.resolution || resolution;
  const readingStage = resl?.reading_stage;
  ...
  const displayStage = (
    normalizedStage === 'SUBMITTED' && resl?.session_id_first_reading
      ? 'RECORD_SESSION'
      : normalizedStage === 'COMMITTEE_REPORT_SUBMITTED' && resl?.committee_report_id
        ? 'ASSIGN_SECOND_SESSION'
      : normalizedStage === 'ASSIGN_SECOND_SESSION' && resl?.session_id_second_reading
  ...
  return (
    <div className="resolution-workflow">
          ...
          {isRejected && (
            <div className="lw-step rejected">
              ...
              <div className="lw-label">
                ...
                <span className="lw-stage-desc">{resl?.rejection_reason || "Measure was rejected"}</span>
              </div>
            </div>
          )}
        </div>
        ...
        {activeAction === 'assign-committee' && (
          <div className="status-modal-overlay">
            <div className="status-modal">
              ...
              <p>Select a committee to study and deliberate on this resolution.</p>
              <div className="form-group">
                <label>Committee <span className="required">*</span></label>
                <select value={form.committee_id || resl?.committee_id || ''} onChange={e => setField('committee_id', e.target.value)} disabled={submitting}>
      ...
      <section className="workflow-section">
        ...
        <div className={["lw-stage-card", isRejected ? "rejected" : ""].filter(Boolean).join(" ")}>
          ...
          <div>
            ...
            <p>{isRejected
              ? (resl?.rejection_reason || "No reason provided")
        ...
        <section className="workflow-section">
          ...
          {votingLoading ? (
            ...
            <div className="lw-electronic-voting">
                      ...
                      {['Yes', 'No', 'Abstain'].map(option => (
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setError('');
                            try {
                              await api.post(`/resolutions/${resolutionId}/cast-vote`, { vote_option: option });
                ...
                <div className="lw-close-voting" style={{ marginTop: '1rem' }}>
                  <button
                    ...
                    onClick={async () => {
                      ...
                      setError('');
                      try {
                        await api.post(`/resolutions/${resolutionId}/close-voting`);
                        setActiveAction(null);
                        await fetchData();
                        await fetchVotingStatus();
                        onStatusUpdate?.();
              ...
              {votingStatus.votingSession?.status === 'closed' && resl?.voting_results && (
                <div className={["lw-vote-results", resl.voting_results.passed ? "passed" : "failed"].join(" ")} style={{ marginTop: '1rem' }}>
                  <span className="lw-vote-result-label">{resl.voting_results.passed ? "✅ PASSED" : "❌ FAILED"}</span>
                </div>
              )}
            </div>
          ) : (
      ...
      {/* Voting Results (for stages after voting) */}
      {resl?.voting_results && normalizedStage !== 'THIRD_READING_VOTING' && !votingStatus?.votingSession && (
        <section className="workflow-section">
          <h3>🗳️ Voting Results</h3>
          <div className={["lw-vote-results", resl.voting_results.passed ? "passed" : "failed"].join(" ")}>
            <span className="lw-vote-result-label">{resl.voting_results.passed ? "✅ Passed" : "❌ Failed"}</span>
            <div className="lw-vote-counts">
              <span className="lw-vote-yes">Yes: <strong>{resl.voting_results.yes_count}</strong></span>
              <span className="lw-vote-no">No: <strong>{resl.voting_results.no_count}</strong></span>
              <span className="lw-vote-abs">Abstain: <strong>{resl.voting_results.abstain_count}</strong></span>
            </div>
          </div>
        </section>
      )}
      ...
      {!isRejected && (
        <section className="workflow-section">
          ...
          {availableActions.includes('assign-committee') && (
            <button className="lw-action-btn lw-assign-committee" onClick={() => handleActionClick('assign-committee')}>
              {ACTION_LABELS['assign-committee']?.emoji} {ACTION_LABELS['assign-committee']?.label}
            </button>
          )}
          {availableActions.includes('create-meeting') && (
            <button className="lw-action-btn lw-create-meeting" onClick={() => handleActionClick('create-meeting')}>
          ...
          {availableActions.includes('admin-override-session') && (
            <button className="lw-action-btn lw-executive-rejection" onClick={() => handleActionClick('admin-override-session')}>
          ...
          {availableActions.includes('second-reading') && (
            <button className="lw-action-btn lw-second-reading" onClick={() => handleActionClick('second-reading')}>
          ...
          {availableActions.includes('executive-approval') && (
            <button className="lw-action-btn lw-executive-approval" onClick={() => handleActionClick('executive-approval')}>
          ...
          {availableActions.includes('executive-rejection') && (
            <button className="lw-action-btn lw-executive-rejection" onClick={() => handleActionClick('executive-rejection')}>
              {ACTION_LABELS['executive-rejection']?.emoji} {ACTION_LABELS['executive-rejection']?.label}
            </button>
          )}
          {availableActions.includes('post-publicly') && (
            <button className="lw-action-btn lw-post-publicly" onClick={() => handleActionClick('post-publicly')}>
              {ACTION_LABELS['post-publicly']?.emoji} {ACTION_LABELS['post-publicly']?.label}
            </button>
          )}
          {availableActions.includes('mark-effective') && (
            <button className="lw-action-btn lw-mark-effective" onClick={() => handleActionClick('mark-effective')}>
