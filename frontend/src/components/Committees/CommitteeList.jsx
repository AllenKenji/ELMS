...
export default function CommitteeList() {
  ...
  const canCreate = ['Admin', 'Vice Mayor'].includes(user?.role);
  // Only Admin or the assigned chairperson can edit
  const canEdit = (committee) => {
    if (!user) return false;
    if (user.role === 'Admin') return true;
    if (committee && String(committee.chair_id) === String(user.id)) return true;
    return false;
  };
  const canDelete = ['Admin'].includes(user?.role);

  // Helper to fetch full committee details and open edit form
  const handleEditCommittee = async (committee) => {
    try {
      const res = await api.get(`/committees/${committee.id}`);
      const {data} = res;
      // Map members to member_ids and extract vice_chair_id
      // Only include members with role 'Member' (exclude Chair and Vice Chair)
      const member_ids = Array.isArray(data.members)
        ? data.members.filter(m => m.role === 'Member').map(m => m.user_id)
        : [];
      const viceChair = Array.isArray(data.members)
        ? data.members.find(m => m.role === 'Vice Chair')
        : null;
      // Use type/scope from data if present, else fallback to empty string
      setEditingCommittee({
        ...data,
        member_ids,
        vice_chair_id: data.vice_chair_id || (viceChair ? viceChair.user_id : ''),
        type: data.type || '',
        scope: data.scope || '',
      });
      setShowForm(true);
    } catch (err) {
      setError('Failed to load committee details for editing');
      console.error('Error:', err);
    }
  };
    ...
    <div className="committee-list-container">
      ...
      {selectedCommittee && (
        <CommitteeDetails
          ...
          onEdit={async () => {
            await handleEditCommittee(selectedCommittee);
            setSelectedCommittee(null);
          }}
          onDelete={() => {
